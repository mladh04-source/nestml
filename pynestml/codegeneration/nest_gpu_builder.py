...
class NESTGPUBuilder(Builder):
    ...
    def __init__(self, options: Optional[Mapping[str, Any]] = None):
        ...
        if (not self.option_exists("nest_gpu_path")) or (not self.get_option("nest_gpu_path")):
            ...
            Logger.log_message(
                None,
                -1,
                "The NEST-GPU path was automatically detected as: " + nest_gpu_path,
                None,
                LoggingLevel.INFO
            )
        else:
            ...
            Logger.log_message(
                None,
                -1,
                "The NEST-GPU path was set to: " + str(nest_gpu_path),
                None,
                LoggingLevel.INFO
            )

        # build path sicher setzen
        if (not self.option_exists("nest_gpu_build_path")) or (self.get_option("nest_gpu_build_path") is None):
            build_path = os.path.join(nest_gpu_path, "build")
            self.set_options({"nest_gpu_build_path": build_path})

        # install path sicher setzen
        if (not self.option_exists("nest_gpu_install_path")) or (self.get_option("nest_gpu_install_path") is None):
            install_path = os.path.join(nest_gpu_path, "install")
            self.set_options({"nest_gpu_install_path": install_path})
    ...
    def build(self) -> None:
        ...
        if not os.path.isdir(target_path):
            raise InvalidPathException('Target path (' + str(target_path) + ') is not a directory!')
        ...
        try:
            ...
            raise GeneratedCodeBuildException(
                "Error occurred during build directory creation! More detailed error messages can be found in stdout."
            )
